...
close all;
%% 數學範例軌跡(兩橢圓形、一圓形)
% sim trajectory : ellipse 理想軌跡
a_s = 2*pi;
b_s = 1*pi;
t_s = linspace(pi,-pi,1000);
x_s = a_s*cos(t_s);
y_s = b_s*sin(t_s);
sim_tr = [x_s', y_s'];

% real trajectory 1 : ellipse*2 放大兩倍之真實軌跡1
a = 2*pi;
b = 1*pi;
t_r1 = linspace(pi,-pi,50);
x = a*cos(t_r1);
y = b*sin(t_r1);
z = [0,0,2,0];
ori_tr = [x',y']
...
% real trajectory 1 : circle 改變輪廓之真實軌跡2
t_r2 = linspace(pi,-pi,50);
x_r2 = 2*pi*cos(t_r2);
y_r2 = 2*pi*sin(t_r2);
real_tr2 = [x_r2', y_r2'];
...
figure
hold on
plot(sim_tr(:,1), sim_tr(:,2),'-','color','k');
plot(real_tr1(:,1), real_tr1(:,2),'x','color',[0 0.4470 0.7410]);
plot(real_tr2(:,1), real_tr2(:,2),'s','color',[0.8500 0.3250 0.0980]);
...
lb_tran1 = [-delta_x1*5 -delta_y1*5 0 -5];
ub_tran1 = [delta_x1*5 delta_y1*5 5 5];
% lb_tran1 = [-15 -15 0 -15];
% ub_tran1 = [15 15 5 15];
...
bias_result1 = sum(bias_result_n1([1,2,3,4,5,6]).*w_nor);
...
lb_tran2 = [-delta_x2*5 -delta_y2*5 0 -5];
ub_tran2 = [delta_x2*5 delta_y2*5 5 5];
% lb_tran2 = [-15 -15 0 -15];
% ub_tran2 = [15 15 5 15]
...
bias_result2 = sum(bias_result_n2([1,2,3,4,5,6]).*w_nor);
...
figure
hold on
plot(x_tran, y_tran,'.','color','k');
plot(real2_eq(:,1), real2_eq(:,2),'s','color',[0.8500 0.3250 0.0980]);
% plot(sim_eq(:,1), sim_eq(:,2),':s','color',[0.8500 0.3250 0.0980]);
scatter(x_tran(1,1), y_tran(1,1),sz_s,'k','filled');
scatter(real2_eq(1,1), real2_eq(1,2),sz,[0.8500 0.3250 0.0980],'filled');
legend('$T^s$ Transform','$T^{r}_2$','interpreter','latex');
xlabel('X-Coordinate');
ylabel('Y-Coordinate');
xlim([-4*pi,4*pi])
grid on
